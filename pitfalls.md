...
只把当前 `.think-text` 也滚到底还不够。实测会出现更怪的形态:
外层右侧滚动条已经在底部,「回到底部」按钮点击后能跳到真正最新内容,
但滚轮一动又跳回旧的截断位置。

原因仍是两个滚动坐标同时存在:活动推理块内部有自己的 `overflow-y:auto`,
而外层 `.stream` 也在滚。按钮和滚轮命中的元素不一致时,用户看到的就是
「按钮的底部」和「滚动条的底部」不是同一个地方。

修法:活动中的推理块加 `.live`,取消内部 `max-height/overflow`,
让它真实撑高外层 `.stream`;等推理收口时移除 `.live` 并折叠。
这样生成期间只有外层一个滚动边界。按钮兜底再把所有已展开的内部滚动区
也滚到底,覆盖用户手动展开历史推理块的情况。
